...
struct SegTree{
    ...
    pii query(int idx , int l , int r , int ql , int qr){
        ...
        if(ql <= l && r <= qr){
            return {pref[idx] , tree[idx]};
        }
        int mid = (l + r) / 2;
        pii left = query(2 * idx , l , mid , ql , qr);
        pii right = query(2 * idx + 1 , mid + 1 , r , ql , qr);
        int temp1 = left.ff;
        int temp2 = left.ss + right.ff;
        return {max(temp1 , temp2) , left.ss + right.ss};
    }
};
void solve() {
    int n , q;
    ...
    for(int i = 0 ; i < q ; i++){
        ...
        if(x == 1){
            ...
            int a , b;
            cin >> a >> b;
            pii ans = st.query(1 , 0 , n - 1 , a - 1 , b - 1);
            cout << ans.ff << nline;
        }
    }
}
