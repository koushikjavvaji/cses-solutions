...
struct LazySegTree {
    ...
    vector<bool>isLazy;
    vector<int>opsInd;
    LazySegTree(int n , vector<int>& a){
        this -> n = n;
        tree.resize(4 * n , 0);
        lazy.resize(4 * n , 0);
    ...
    void push(int idx , int l , int r) {
        if(!isLazy[idx] || l == r) return;

        int mid = (l + r) / 2;

        apply(2 * idx , l , mid , lazy[idx] , opsInd[idx]);
        apply(2 * idx + 1 , mid + 1 , r , lazy[idx] , opsInd[idx]);
    ...
    void apply(int idx , int l , int r , int val , int ops) {
        if(ops == 2) { // ASSIGN
            tree[idx] = val * (r - l + 1);
            lazy[idx] = val;
            opsInd[idx] = 2;
            ...
            if(isLazy[idx] && opsInd[idx] == 2) {
                // add on top of assign
                lazy[idx] += val;
            } 
            else {
                lazy[idx] += val;
    ...
    void update(int idx , int l , int r , int ql , int qr , int val , int ops) {
        if(l > qr || r < ql) {
            return;
        }
        if(l >= ql && r <= qr){
            apply(idx , l , r , val , ops);
            return;
        }
        push(idx , l , r);
        int mid = (l + r) / 2;
        update(2 * idx , l , mid , ql , qr , val , ops);
        update(2 * idx + 1, mid + 1, r, ql, qr, val , ops);
...
void solve() {
    ...
    for(int i = 0 ;i < q ; i++){
        ...
        if(op == 1){
            int a , b , u;
            cin >> a >> b >> u;
            st.update(1 , 0 , n - 1 , a - 1 , b - 1 , u , 1);
        }else if(op == 2){
            int a , b , u;
            cin >> a >> b >> u;
            st.update(1 , 0 , n - 1 , a - 1 , b - 1 , u , 2);
        }else{
            int a , b;
            cin >> a >> b;
            cout << st.query(1 , 0 , n - 1 , a - 1 , b - 1) << nline;
        }
    }
}
int32_t main() {
