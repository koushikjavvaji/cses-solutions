...
struct SegTree {
    ...
    int update(int idx , int l , int r , int val) {
        if(l == r){
            tree[idx] -= val;
            return l + 1;
        }
        int left = tree[2 * idx] , right = tree[2 * idx + 1];
        int mid = (l + r) / 2;
        int res;
        if(left >= val){
            res = update(2 * idx , l , mid , val);
        }else if(right >= val){
            res = update(2 * idx + 1 , mid + 1 , r , val);
        }else{
            return 0;
        }
        tree[idx] = max(tree[idx * 2] , tree[idx * 2 + 1]);
...
void solve() {
    ...
    for(int i = 0 ;i < m ; i++){
        cin >> r[i];
        int maxi = st.query(1 , 0 , n - 1 , 0 , n - 1);
        if(maxi < r[i]){
            cout << 0 << " ";
        }else{
            cout << st.update(1 , 0 , n - 1 , r[i]) << " ";
        }
    }
    cout << nline;
}
