...
struct LazySegTree {
    ...
    void push(int idx , int l , int r) {
        if(lazyA[idx] == 0 && lazyB[idx] == 0) return;
    ...
    void apply(int idx , int l , int r , int A , int B) {
            
        long long len = r - l + 1;
        long long sum_i = 1LL * (l + r) * len / 2;
    
        tree[idx] += (A * sum_i + B * len);
            
        lazyA[idx] += A;
        lazyB[idx] += B;
    }
    
    void update(int idx , int l , int r , int ql , int qr , int A , int B) {
        if(l > qr || r < ql) return;
...
void solve() {
    ...
    for(int i = 0 ;i < q ; i++){
        ...
        if(op == 1){
            int a , b;
            cin >> a >> b;
            st.update(1 , 0 , n - 1 , a - 1, b - 1, 1 , 2 - a);
        }else{
            int a , b;
            cin >> a >> b;
