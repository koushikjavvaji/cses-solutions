...
void solve(){
    ...
    for(int i = 0 ; i < q ; i++){
        ...
        auto &stVal = mpst[val];
        auto &stInit = mpst[init];

        auto it = stVal.upper_bound(k);
        if(it != stVal.end()){
            st.update(1 , 0 , n - 1 , k , *it);
        }else{
            st.update(1 , 0 , n - 1 , k , INF);
        }

        a[k] = u;

        auto it1 = stInit.lower_bound(k);
        auto it2 = stInit.upper_bound(k);

        if(it1 != stInit.begin()){
            --it1;
            if(it2 != stInit.end()){
                st.update(1 , 0 , n - 1 , *it1 , *it2);
            }else{
                st.update(1 , 0 , n - 1 , *it1 , INF);
            }
        }
        ...
        auto it3 = stVal.find(k);
        if(it3 != stVal.begin()){
            auto prev = it3;
            --prev;
            st.update(1 , 0 , n - 1 , *prev , k);
        }
    }else{
          int curr =  st.query(1 , 0 , n - 1 , x1 - 1, y - 1);
          debug(curr);
