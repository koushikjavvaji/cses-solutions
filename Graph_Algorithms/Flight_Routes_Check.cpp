...
*               Main Function                   *
*************************************************/
void dfs(int curr, vector<vector<int>>& edges, vector<int>& vis, int& cnt) {
    vis[curr] = 1;
    cnt++;
    for (auto it : edges[curr]) {
        if (!vis[it]) {
            dfs(it, edges, vis, cnt);
        }
    }
}
void solve() {
    int n , m;
    cin >> n >> m;
    vector<vector<int>>edges1(n),edges2(n);
    for(int i = 0 ;i < m ;i++)
    {
        int u , v;
        cin >> u >> v;
