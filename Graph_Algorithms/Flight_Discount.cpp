...
void solve() {
    ...
    while(!st.empty()){
        auto curr = *st.begin();
        st.erase(curr);
        for(auto it : edges[curr.ss.ff]){
            if(curr.ss.ss == 0){
                int dis = curr.ff + ((it.ss) / 2);
                if(dis < dist[it.ff][1]){
                    st.erase({dist[it.ff][1] , {it.ff , 1}});
                    dist[it.ff][1] = dis;
                    st.insert({dis , {it.ff , 1}});
                }
            }
            int dis = curr.ff + ((it.ss));
                if(dis < dist[it.ff][curr.ss.ss]){
                    st.erase({dist[it.ff][curr.ss.ss] , {it.ff , curr.ss.ss}});
                    dist[it.ff][curr.ss.ss] = dis;
                    st.insert({dis , {it.ff , curr.ss.ss}});
                }
