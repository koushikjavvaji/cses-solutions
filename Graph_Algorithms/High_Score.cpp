...
#define INF LLONG_MIN // Updated to LLONG_MIN
#define nline "\n"
#define pb push_back
#define ppb pop_back
#define mp make_pair
...
void solve() {
    ...
    for (int j = 1; j <= (n + 1); j++) {
        for (int i = 0; i < n; i++) {
            for (auto it : edges[i]) {
                if (dist[i] != INF) { // Check to prevent overflow
                    dist[it.ff] = max(dist[it.ff], dist[i] + it.ss);
                }
            }
        }
        if (j == n) temp = dist;
    ...
    if (temp != dist) {
        ...
        vector<int> visT(n, 0);
        function<void(int)> f1 = [&](int curr) -> void {
            visT[curr] = 1;
            if (incycle[curr] == 1) {
                temp[curr]++;
            }
