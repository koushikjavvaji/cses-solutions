...
*               Main Function                   *
*************************************************/
// state : best ans possibile form 0 to i where the current value is x
void solve() {
    int n, m;
    cin >> n >> m;
    vector<int> x(n);
    ...
    function<int(int, int)> f = [&](int ind, int num) -> int {
        if (ind == n) return 1; // Base case: reached the end
        if (num < 1 || num > m) return 0; // Invalid number
        if (dp[ind][num] != -1) return dp[ind][num]; // Memoized result
 
        int res = 0;
        if (x[ind] == 0) {
            if (num > 1) res = mod_add(res, f(ind + 1, num - 1), MOD);
            res = mod_add(res, f(ind + 1, num), MOD);
            if (num < m) res = mod_add(res, f(ind + 1, num + 1), MOD);
        }
        else
        {
            if(abs(num - x[ind]) <= 1)
        ...
        return dp[ind][num] = res;
    };
 
    int result = 0;
    if (x[0] == 0) {
        for (int i = 1; i <= m; i++) {
            result = mod_add(result, f(1, i), MOD);
        }
    } else {
        result = f(1, x[0]);
    }
