...
*               Main Function                   *
*************************************************/
// state : best ans possibile from ind1 to n and ind2 to m
void solve() {
    string s1,s2;
    cin >> s1 >> s2;
    int n = s1.length();
    ...
    function<int(int , int)> f = [&](int ind1 ,int ind2) -> int {
        if(ind1 == n) return abs(m - ind2);
        if(ind2 == m) return abs(n - ind1);
        if(dp[ind1][ind2] != -1) return dp[ind1][ind2];
        int ans = LLONG_MAX;
        if(s1[ind1] == s2[ind2]) ans = f(ind1 + 1 , ind2 + 1);
        else
        {
            int option1 = f(ind1 + 1 , ind2) + 1;
            int option2 = f(ind1 , ind2 + 1) + 1;
            int option3 = f(ind1 + 1 ,ind2 + 1) + 1;
