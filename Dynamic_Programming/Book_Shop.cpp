...
void solve(){
    ...
    //     if(num == 0 || ind == n) return 0;
    //     if(dp[ind][num] != -1) return dp[ind][num];
    //     return dp[ind][num] = max(f(ind + 1,num) , (num >= h[ind]) ? f(ind + 1,num - h[ind]) + s[ind] : 0);
    // };
    // cout << f(0,x) << nline;
    vector<int>lastrow(x + 1);
    for(int i = 0 ;i <= x ; i++)
    {
       // dp[n][i] = 0;
       lastrow[i] = 0;
    }
    vector<int>currentrow(x + 1);
    for(int i = n - 1 ;i >= 0 ; i--)
    {
        for(int j = 0 ; j <= x ; j++)
        {
            // dp[i][j] = max(dp[i + 1][j] ,(j - h[i]) >= 0 ? dp[i + 1][j - h[i]] + s[i] : 0);
            currentrow[j] = max(lastrow[j] , (j - h[i]) >= 0 ? lastrow[j - h[i]] + s[i] : 0);
        }
        lastrow = currentrow;
    }
    cout << currentrow[x] << nline;
}
int32_t main() {
    fastio();
    #ifndef ONLINE_JUDGE
